...
function generateAcronym(element) {
  const ignoredWords = [
    'the',
    'of',
    'in',
  ...
  if (element.length > 30) {
    ...
    words = words.filter((word) => !ignoredWords.includes(word));
...
function generateBC(url, separator) {
  ...
  const matchedSegments = matches.map((match) => match.replace('/', ''));

  let urlSegments = matchedSegments.filter(
  ...
  let breadcrumbParts = ['<a href="/">HOME</a>'];
  let currentPath = '';
  ...
  urlSegments.forEach((segment, index) => {
    ...
    if (index === urlSegments.length - 1) {
      ...
      if (segment.includes('-')) {
        segment = generateAcronym(segment);
      }
      breadcrumbItem = `<span class="active">${segment.toUpperCase()}</span>`;
    } else {
      // Intermediate segment, add as a link
      if (segment.includes('-')) {
        segment = generateAcronym(segment);
      }
      breadcrumbItem = `<a href="/${currentPath}/">${segment.toUpperCase()}</a>`;
    }

    breadcrumbParts.push(breadcrumbItem);
  });

  console.log('breadcrumbParts', breadcrumbParts.join(separator));
  return breadcrumbParts.join(separator);
}
